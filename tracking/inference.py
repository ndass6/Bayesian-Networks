...
class ParticleFilter(InferenceModule):
    ...
    def elapseTime(self, gameState):
        ...
        for pos in self.particles:
            newParticles.append(util.sample(self.getPositionDistribution(self.setGhostPosition(gameState, pos))))
        ...
        ** Remember ** We store particles as tuples, but to edit a specific particle,
        ...
        for ghost in range(self.numGhosts):
            if noisyDistances[ghost] == None:
                ghostIndexJail.append(ghost)

        newParticles = util.Counter()
        for particle in self.particles:
            for ghost in ghostIndexJail:
                particle = self.getParticleWithGhostInJail(particle, ghost)
            probability = 1
            for ghost in range(self.numGhosts):
                if ghost not in ghostIndexJail:
                    probability *= emissionModels[ghost][util.manhattanDistance(particle[ghost], pacmanPosition)]
            newParticles[particle] += probability
        self.beliefs = newParticles

        if not newParticles.totalCount():
            self.initializeParticles()
        else:
            self.beliefs.normalize()
            for pos in range(len(self.particles)):
                self.particles[pos] = util.sample(self.beliefs)
        ...
        **Note** that you may need to replace "prevGhostPositions" with the
        ...
        for oldParticle in self.particles:
            ...
            for g in range(self.numGhosts):
                 newParticle[g] = util.sample(getPositionDistributionForGhost(setGhostPositions(gameState, newParticle), g, self.ghostAgents[g]))
